/**
 * Funzione di hashing SHA-256 in TypeScript puro.
 * Consente l'hashing sicuro delle password senza dipendenze native esterne,
 * garantendo il funzionamento cross-platform su iOS, Android, Web e Cloud Functions.
 */
export function hashPassword(password: string): string {
  ...
  for (let i = 0; i < words.length; i += 16) {
    ...
    while (w.length < 64) {
      const s0 = rotateRight(7, w[w.length - 15]) ^ rotateRight(18, w[w.length - 15]) ^ (w[w.length - 15] >>> 3);
      const s1 = rotateRight(17, w[w.length - 2]) ^ rotateRight(19, w[w.length - 2]) ^ (w[w.length - 2] >>> 10);
